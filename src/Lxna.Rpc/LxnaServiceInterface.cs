...
namespace Lxna.Rpc
{
    public sealed class LxnaServiceInterface
    {
        ...
        private LockedHashDictionary<uint, Action<ReadOnlySequence<byte>>> _queueMap = new LockedHashDictionary<uint, Action<ReadOnlySequence<byte>>>();
        private TaskManager _receiveTaskManager;

        private readonly object _lockObject = new object();
        private volatile bool _disposed;

        public LxnaServiceInterface(INonblockingConnection connection)
        {
            ...
            while (!token.IsCancellationRequested)
            {
                _connection.DequeueAsync((sequence) =>
                {
                    try
                    {
                        ...
                        var header = LxnaRpcResponseHeader.Formatter.Deserialize(reader, 0);

                        if(_queueMap.TryGetValue(header.Id, out var action))
                        {
        ...
        private TResult Send<TArgument, TResult>(LxnaRpcRequestType type, TArgument argument)
            where TArgument : RocketPackMessageBase<TArgument>
            where TResult : RocketPackMessageBase<TResult>
        {
            uint id = this.CreateId();
            var taskCompletionSource = new TaskCompletionSource<TResult>();

            _queueMap.Add(id, (sequence) =>
            {
                var reader = new RocketPackReader(sequence, BufferPool.Shared);
                var header = LxnaRpcResponseHeader.Formatter.Deserialize(reader, 0);

                if (header.Type == LxnaRpcResponseType.Result)
                {
                    var result = RocketPackMessageBase<TResult>.Formatter.Deserialize(reader, 0);
                    taskCompletionSource.SetResult(result);
                }else if (header.Type == LxnaRpcResponseType.)
            });
        ...
        {
            public LxnaRpcResponseType Type { get; set; }
