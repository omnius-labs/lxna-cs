...
namespace Omnius.Lxna.Ui.Desktop.Controls;
...
public class FileViewControlViewModel : AsyncDisposableBase
{
    ...
    private readonly ObservableCollection<ItemViewModel> _currentItemModels = new();
    ...
    private async void Init()
    {
        ...
        {
            var model = new DirectoryViewModel(drive, _fileSystem);
    ...
    {
        if (item is not ItemViewModel model) return;
    ...
    {
        if (item is ItemViewModel model)
        {
    ...
    private async Task RefreshAsync(DirectoryViewModel selectedDirectory)
    {
    ...
    private async Task RefreshCurrentItemModelsAsync(NestedPath path, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            await Avalonia.Threading.Dispatcher.UIThread.InvokeAsync(() =>
            {
                ...
                {
                    _currentItemModels.Add(new ItemViewModel(dirPath));
                }

                foreach (var filePath in files)
                {
                    _currentItemModels.Add(new ItemViewModel(filePath));
                }
            });
