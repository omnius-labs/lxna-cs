...
{
    private sealed class Extractor : DisposableBase, IArchiveFileExtractor
    {
        ...
        public static async ValueTask<IArchiveFileExtractor> CreateAsync(string path, IBytesPool bytesPool, CancellationToken cancellationToken = default)
        {
            var result = new Extractor(path, bytesPool);
            await result.InitAsync();

            return result;
        }

        private Extractor(string path, IBytesPool bytesPool)
        {
