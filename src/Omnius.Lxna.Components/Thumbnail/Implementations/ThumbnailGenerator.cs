...
public sealed class ThumbnailGenerator : AsyncDisposableBase, IThumbnailGenerator
{
    ...
    public static async ValueTask<IThumbnailGenerator> CreateAsync(IBytesPool bytesPool, ThumbnailGeneratorOptions options)
    {
        var result = new ThumbnailGenerator(bytesPool, options);
        await result.InitAsync();

        return result;
    }

    private ThumbnailGenerator(IBytesPool bytesPool, ThumbnailGeneratorOptions options)
    {
        _bytesPool = bytesPool;
        _options = options;

        _thumbnailGeneratorRepository = new ThumbnailGeneratorRepository(Path.Combine(_options.ConfigDirectoryPath, "thumbnails.db"), _bytesPool);
    }
    ...
    public async ValueTask<ThumbnailGeneratorGetThumbnailResult> GetThumbnailAsync(IFile file, ThumbnailGeneratorGetThumbnailOptions options, bool cacheOnly = false, CancellationToken cancellationToken = default)
    {
        ...
        // Picture
        {
            var result = await this.GetPictureThumbnailAsync(file, options, cancellationToken).ConfigureAwait(false);
            if (result.Status == ThumbnailGeneratorGetThumbnailResultStatus.Succeeded) return result;
        }

        // Movie
        {
            var result = await this.GetMovieThumbnailAsync(file, options, cancellationToken).ConfigureAwait(false);
            if (result.Status == ThumbnailGeneratorGetThumbnailResultStatus.Succeeded) return result;
        }
    ...
    private async ValueTask<ThumbnailGeneratorGetThumbnailResult> GetPictureThumbnailAsync(IFile file, ThumbnailGeneratorGetThumbnailOptions options, CancellationToken cancellationToken = default)
    {
        ...
        if (!_pictureTypeExtensionList.Contains(ext)) return new ThumbnailGeneratorGetThumbnailResult(ThumbnailGeneratorGetThumbnailResultStatus.Failed);
    ...
    private async ValueTask<ThumbnailGeneratorGetThumbnailResult> GetMovieThumbnailAsync(IFile file, ThumbnailGeneratorGetThumbnailOptions options, CancellationToken cancellationToken = default)
    {
        ...
        var ext = file.LogicalPath.GetExtension().ToLower();
        if (!_movieTypeExtensionList.Contains(ext)) return new ThumbnailGeneratorGetThumbnailResult(ThumbnailGeneratorGetThumbnailResultStatus.Failed);
        try
        {
            var fileLength = await file.GetLengthAsync(cancellationToken);
            var fileLastWriteTime = await file.GetLastWriteTimeAsync(cancellationToken);
